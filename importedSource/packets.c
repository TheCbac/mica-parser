...
*   packets_ERR_SUCCESS            | On Successful init
*******************************************************************************/
uint32_t packets_processRxByte(packets_BUFFER_FULL_S* buffer) {
     uint32_t error = packets_ERR_SUCCESS;
...
/*******************************************************************************
* Function Name: packets_processRxQueue()
****************************************************************************//**
* \brief
...
*   packets_ERR_SUCCESS            | On Successful init
*******************************************************************************/
uint32_t packets_processRxQueue(packets_BUFFER_FULL_S* buffer){
    /* Get the number of pending bytes */
    uint32_t pendingBytes = buffer->comms.rxGetBytesPending();
    uint32_t i;
    uint32_t err = packets_ERR_SUCCESS;
    /* Procces each byte */
    for(i = ZERO; i< pendingBytes; i++){
        err |= packets_processRxByte(buffer);
        /* If the packet is complete Start Action on it*/
        if(buffer->receive.bufferState == packets_BUFFER_RECEIVE_COMPLETE){
            /* Parse the packet */
            err |= packets_parsePacket(buffer);
            if(!err){
                if(buffer->comms.ackCallback != NULL && buffer->comms.ackCallback != NULL){
                    /* Local Reference */
                    packets_PACKET_S *rxPacket = &(buffer->receive.packet);
                    /* Is the packet an ACK or CMD packet */
                    if(rxPacket->flags & packets_FLAG_ACK){
                        /* Call the ack callback */
                        err |= buffer->comms.ackCallback(rxPacket);
                    } else {
                    /* Handle the cmd packet */
                        err |= packets_handleCmdPacket(buffer);
                    }
                } else {
                    err |= packets_ERR_CALLBACK;
                }
            }
...
uint32_t packets_parsePacket(packets_BUFFER_FULL_S* buffer) {
    ...
    uint8_t startByte = rxBuffer->buffer[packets_INDEX_START];
...
/*******************************************************************************
* Function Name: packets_handleCmdPacket()
****************************************************************************//**
* \brief
...
*******************************************************************************/
    uint32_t packets_handleCmdPacket(packets_BUFFER_FULL_S* buffer){
    packets_PACKET_S* rxPacket = &(buffer->receive.packet);
    packets_PACKET_S* txPacket = &(buffer->send.packet);
    ...
    if(validateErr ==packets_ERR_SUCCESS){
        ...
        if( !(buffer->receive.packet.flags & packets_FLAG_NO_ACK)){
            /* Send the response packet */
            responseErr = packets_sendPacket(buffer);
        }
